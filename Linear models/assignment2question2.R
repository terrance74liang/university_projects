...
bounds_yh_new <- function(input, alpha_half, dfm) {
    ...
    bounds <- c(
        least_squares(input) - qt(alpha_half, dfm, lower.tail = FALSE) * sqrt(s_square),
        least_squares(input) + qt(alpha_half, dfm, lower.tail = FALSE) * sqrt(s_square)
    )
    return(bounds)
}
...
bounds_yh_new_m(2, 0.005, 8, 3)

bounds_yh_new_m(2, 0.005, 8, 3) * 3

bounds_cbands <- function(input, alpha_half, dfm, dfm2) {
    MSE <- (sum((y_axis - least_squares(x_axis))^2) / (nrow(data) - 2))
    s_square <- MSE * ((1 / nrow(data)) + (((input - x_bar)^2) / sum_den))
    bounds <- c(
        least_squares(input) - sqrt(2 * qf(alpha_half, dfm, dfm2, lower.tail = FALSE)) * sqrt(s_square),
        least_squares(input) + sqrt(2 * qf(alpha_half, dfm, dfm2, lower.tail = FALSE)) * sqrt(s_square)
    )
    return(bounds)
}

bounds_cbands(2, 0.01, 2, 8)
bounds_cbands(4, 0.01, 2, 8)
